...
from app.libs.error_code import Success, jsonify
...
def update_route_tree():
    """拖动修改路由结构"""
    RouteDao.change_route(request.json['route_tree'])
    return Success()


@api.route('/<id>', methods=['GET'])
@api.doc(args=['path.id'], auth=True)
# @auth.admin_required
...
@api.doc(args=api_doc_args, auth=True)
# @auth.admin_required
def update_route_node_by_id(id):
    """按ID修改路由节点"""
...
@api.doc(args=api_doc_args[2:], auth=True)
# @auth.admin_required
